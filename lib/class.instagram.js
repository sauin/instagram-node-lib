...
(function() {
  var APIClient, InstagramAPI, querystring, crypto;
  ...
  InstagramAPI = (function() {
    ...
    */
    ...
    InstagramAPI.prototype._request = function(params) {
      ...
      var sig = function (secret, path, parameters) {
        if (!parameters) {
            var list = path.split('?');
            path = list[0];
            parameters = list[1];
        ...
        return crypto
                ...
                .update(path + '|' + parameters.split('&').sort().join('|'))
                .digest('hex');
      }
      ...
      if (options.method !== 'GET') {
        options['headers']['X-Insta-Forwarded-For'] = options['service_ip'] + '|';
        options['headers']['X-Insta-Forwarded-For'] += crypto.createHmac('sha256', this._config['client_secret']).update(options['service_ip']).digest('hex');
        if  (params.post_data != null) {
            post_data = this._to_querystring(params.post_data);
            post_data += '&sig=' + sig(this._config.client_secret, options.path, post_data);
        }
      }
      if (options.method !== 'POST') {
        post_data = '';
        options.path += '&sig=' + sig(this._config.client_secret, options.path);
      }
      ...
      request = http_client.request(options, function(response) {
        ...
        return response.on('end', function() {
          ...
          try {
            ...
            if (parsedResponse && parsedResponse['code'] >= 400) {
                parsedResponse['meta'] = parsedResponse;
            }
